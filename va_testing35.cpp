...
int main()
{
    ...
    for (int scen=0;scen<numScens;++scen)
    {
        ...
        for (int i=0;i<numPeriods;++i)
        {
            ...
            for (int f=0;f<10;++f)
            {
                pos=i*10*numScens+f*numScens+scen;
                iss>>fundRets[pos];
            }
    ...
    REAL totCOG=0;
    #ifdef openacc
        //#pragma acc parallel loop gang vector(64) reduction(+:totCOG)
        //#pragma acc parallel loop gang vector(64) reduction(+:totCOG) copyin(fundRets[0:numScens*numPeriods*10]) copyin(annDiscRate[0:numScens*numPeriods]) copyin(mort_pre_ret[0:116][0:2]) copyin(mort_post_ret[0:2][0:201][0:numCalYears])
        //#pragma acc  parallel loop gang vector reduction(+:totCOG)  copyin(fundRets[0:numScens*numPeriods*10]) copyin(annDiscRate[0:numScens*numPeriods]) copyin(mort_pre_ret[0:116][0:2]) copyin(mort_post_ret[0:2][0:201][0:numCalYears])  //copyin(sexes[0:numPols]) copyin(productTypes[0:numPols]) copyin(ageYears_ds[0:numPols]) copyin(ageMonths_ds[0:numPols]) copyin(numFunds_ds[0:numPols]) copyin(fundCodes[0:numPols]) copyin(termOSs[0:numPols]) copyin(elapsedMonthss[0:numPols]) copyin(termToNextRenewals[0:numPols]) copyin(renewalTerms[0:numPols]) copyin(baseFee_ds[0:numPols]) copyin(riderFee_ds[0:numPols]) copyin(rollUpRates[0:numPols]) copyin(gbAmt_ds[0:numPols]) copyin(gmwbBalance_ds[0:numPols]) copyin(wbWithdrawalRates[0:numPols]) copyin(withdrawals[0:numPols]) copyin(gteedAnnFacs[0:numPols]) copyin(fundValues[0:numPols]) copyin(fundFees[0:numPols])
        //#pragma acc loop gang worker
        #pragma acc kernels
        #pragma acc loop independent gang worker(4) reduction(+:totCOG) 
    ...
    for (int pol=0;pol<numPols;++pol)
    {
        ...
        for (int scen=0;scen<numScens;++scen)
        {
            ...
            REAL fundCF[numFunds];
            for (int f=0;f<numFunds_d;++f)
                fundCF[f]=fundValue[f];
            ...
            REAL pvCOG=0;
            REAL qx;
            REAL accountValueBeforeGB;
            REAL gbAmt;
            REAL gbAmtCF;
            REAL discFac;
            qx=1-pow(1-mort_pre_ret[ageYears_d][sex],1./12.);
            for (int t=1;t<=termOS;++t)
            {
                int termIFMonths=termIFMonths__1==11?0:++termIFMonths__1;
                accountValueBeforeGB=0;

                REAL fundAfterGrowth[numFunds],riderFee[numFunds],baseFee[numFunds],fundAfterRiskFees[numFunds];
                REAL fees=0;
                //#pragma acc loop independent reduction(+:accountValueBeforeGB) 
                REAL rets[numFunds];
                for (int f=0;f<numFunds;++f)
                {
                    int fpos=(t-1)*10*numScens+f*numScens+scen;
                    rets[f]=fundRets[fpos];
                }
                for (int f=0;f<numFunds_d;++f)
                {
                    fundAfterGrowth[f]=fundCF[f]*rets[fundCode[f]]*(1-fundFee[f]/12);
                ...
                discFac=discFac__1*discFacMonth;
                #ifdef debug
                    if (scen==8&&t==29)
                        printf("","");
                #endif
                ...
                REAL wdlAmt=0;
                REAL gmwbBalance;
                if (hasWithdrawalBen&&t<termOS)
                    wdlAmt=termIFMonths!=0?0:min(wbWithdrawalRate*gbAmt,gmwbBalance__1);

                REAL wdlBen=0;
                if (t<termOS&&hasWithdrawalBen)
                    wdlBen=max(0.0f,wdlAmt-accountValueBeforeGB);
                ...
                if (ageMonths==0)
                    qx=1-pow(1-mort_pre_ret[ageYears][sex],1./12.);
                ...
                for (int f=0;f<numFunds_d;++f)
                    fundCF[f]=fundAfterRiskFees[f]*accountValueAfterGB/(accountValueBeforeGB+eps)*(1-qx);

                REAL survCF=survCF__1*(1-qx);

                pvCOG+=discFac*(livingBen*(1-qx)+deathBen+wdlBen-fees);
                #ifdef debug
                if (incomeBen>0)
                    printf("","");
            ...
            REAL wdlBen=0; 
            if (hasWithdrawalBen)
                wdlBen=max(0.0f,gmwbBalance__1-accountValueBeforeGB);
            REAL mktAnnFac=0;
            REAL incomeBen=0;
            if (hasIB)
            {
                REAL disc=1;
                REAL annDisc=1/(1+annDiscRate[scen*360+termOS-1]);
                for (int i=0;i<55;++i,disc*=annDisc)
                    mktAnnFac+=annSurv[i]*disc;
                incomeBen=max(0.0f,gbAmtCF*mktAnnFac/gteedAnnFac-accountValueBeforeGB*(1-qx));
            }
            REAL matyBen=0;
            if (hasMatyBen)
                matyBen=max(0.0f,gbAmt-accountValueBeforeGB)*(1-qx);
            pvCOG+=discFac*(incomeBen+wdlBen+matyBen);
