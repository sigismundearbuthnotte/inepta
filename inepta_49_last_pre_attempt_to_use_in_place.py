...
calcSettings={"STORE":True,"TYPE":True,"EXPERIENCE":False,"OUTPUT":False,"REBASE":False,"ARRAYED":True,"STAGED":False,"NOTUSEDINBASES":False,"NOTUSEDINEXPERIENCE":False}#does a value follow the setting?
...
bases={}#name->rebased
rebaseTimes=[]
stagingInterval=-1#do full copy and in-place update of store-all every so often; in between, update a staging array (more efficient)
...
class calcInfoObject:
    def __init__(self):
        ...
        self.isStaged=False#for "all", will place past values in staging array first and then do bulk update base on stagingInterval
        self.isArrayed=False
        self.numDims=0
        self.dimSizes=[]
        self.storedForExperience=False#ends up in basisValues and can be used in the experience (e.g. to calculate reserves)
        self.code=[]
        self.initialisation=""#initialisation expression
        self.skipCondition=""#expression for not doing anything
        self.isCall=False
        self.outputMe=False#output from Experience (i.e. what reaches the outside world)
        self.usedForRebasing=False#used as a calc after rebasing, not as part of the usual sequece of calcs
        self.usedInBases=True#for "all", to save space, if not needed in the bases, will not be stored
        self.usedInExperience=True#for "all", to save space, if not needed in Experience (e.g. it's a calc used to produce a reserve), will not be stored
        ...
        if c:
            ...
            if section =="CALC":
                ...
                if l[0:4]=="call":
                    ...
                    if ci.stores!=-1 and ci.isStaged:
                        doErr("Staged applies only to store-all : ", ci.name)
                    ci.code.append(l)
            if section == "TABLES":
                t=tableInfoObject()
                inDims=False
...
for l in ift.readlines():
    ...
    if c:
        if ls[0][0] == "[":
            ...
            continue
        if section=="BASIC":
            if lsu[0] not in set(["MODE","STAGINGINTERVAL"]):
            ...
            if lsu[0]=="STAGINGINTERVAL":
                stagingInterval=int(ls[1])
        if section=="MODELS":
...
off.write("let stagingInterval:i32=("+str(stagingInterval)+")"+nl)
        ...
        for ph in mi.phases.values():
            for ci in ph.values():
                if ci.stores ==-1 :
                    ...
                    comma = ","
                    if ci.isStaged:
                        off.write(comma + ci.name + "_staging:["+str(stagingInterval)+"]" + typ + nl)
        off.write("}\n")
...
for mi in modelInfos.values():
    ...
    for (phName,ph) in mi.phases.items():
        ...
        if count>1:
            ...
            off.write("\tlet as2= as1 with stagingCount=0\n")
            off.write("\tin as2 with state__1 = state_bf\n")
...
#utility to update many values (could have possibly used scatter but decided not to)
if stagingInterval>0:
    off.write("\nlet multiUpdate (source:[]f32) (staging:[]f32) (t:i32) (forb:bool):[]f32=\n")
    off.write("\tlet z1=copy source\n")
    off.write("\tlet z"+str(stagingInterval+2)+"=z1\n")
    off.write("let res=if forb then \n")
    for i in range(1,stagingInterval+1):
        off.write("\tlet z"+str(i+1)+"=z"+str(i)+" with [t+"+str(i-1)+"]=staging["+str(i-1)+"]\n")
    off.write("\t in z"+str(stagingInterval+1)+nl)
    off.write("else\n")
    for i in range(stagingInterval+2,2*stagingInterval+2):
        off.write("\tlet z" + str(i + 1) + "=z" + str(i) + " with [t-" + str(i - stagingInterval -2) + "]=staging[" + str(i - stagingInterval -2) + "]\n")
    off.write("\t in z" + str(2*stagingInterval + 2) + nl)
    off.write("\t in res\n")
...
for mi in modelInfos.values():
    for (phName,phx) in mi.phases.items():
                ...
                for ci in ph.values():
                    if ci.stores==-1:
                        if ph is phx:#only do (possibly expensive) copying etc. if the store-all was actually in this phase
                            if ci.isStaged:#staged: update the staging array
                                off.write("\tlet z_" + ci.name + "__new=copy as.1.state__999." + ci.name + "_staging" + nl)
                                off.write("\tlet " + ci.name + "__new = update z_" + ci.name + "__new as.1.stagingCount as.1.state_new." + ci.name + nl)
                                st999 += comma + ci.name + "_staging=" + ci.name + "__new"
                                comma = ","
                            off.write("\tlet "+ci.name+"=if length as.1.state__999."+ci.name+"==0 then [] else \n")
                            if not ci.isStaged:#not staged: update whole array with copy
                                ...
                                off.write("\t in "+ci.name+"__new\n")
                                st999+=comma+ci.name+"="+ci.name
                            else:#staged: update the array with the staging array
                                off.write("\tlet "+ci.name+"__updated=if as.1.stagingCount!=stagingInterval then as.1.state__999."+ci.name+" else (multiUpdate  as.1.state__999."+ci.name+" as.1.state__999."+ci.name+"_staging"+" as.1.t "+(" true " if mi.phaseDirections[phName]=="forwards" else " false" )+")"+nl)
                                off.write("\tin "+ci.name+"__updated\n")
                                st999+=comma+ci.name+"="+ci.name
                        else:
                            ...
                            comma=","
                            if ci.isStaged:
                                off.write("\tlet " + ci.name + "_staging= as.1.state__999." + ci.name+"_staging" + nl)
                                st999 += comma +ci.name+"_staging=" + ci.name+"_staging"
                        comma = ","
            st999+="}"
        off.write("\tin(\n\t{")
        if mi.hasData:
        ...
        if mi.phaseDirections[phName]=="forwards":
            ...
            off.write("\tt=as.1.t-1,\n")
        off.write("\tstagingCount=if as.1.stagingCount==stagingInterval then 0 else as.1.stagingCount+1,\n")
        ...
        for ph in mi.phases.values():
            for ci in ph.values():
                if ci.stores==-1:
                    ...
                    comma=","
                    if ci.isStaged:
                        off.write(comma + ci.name+"_staging" + "=(zeros1 stagingInterval)")
        off.write("\t},\n")
    for past in range(2,mi.maxStored+1):
        comma=""
