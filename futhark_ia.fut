...
type data_ia={
    ...
    dateLeaveMonth:int,
    NRD:int,
    GMP1:real,--notional slices; will not implement full rules as that would be a hopeless task!  As at data date, say
    GMP2:real,
    GMP3:real,
    GMP4:real,
    GMP5:real,
...
let rbts:*[]bool = replicate numPeriods false 
...
let setDerived_ia(d:data_ia):derived_ia=
    let termOS=maxage*12-(maxi (d.ageYears[0]*12+d.ageMonths[0]) (d.ageYears[0]*12+d.ageMonths[0]))
    ...
    in{termOS=termOS,termToDataDate=termToDataDate}
...
let runOnePeriod_phase0 (as:state_ia_all) :state_ia_all=
    ...
    let qx(as:state_ia_all):state_ia_all=
        let qx_arr1(i1:i32):f32=
            ...
            let qx_ann=tbl_mort_sch[as.basisNum,as.p.scheme,as.p.sex[i1],age,as.state_new.calYear_back]*
...
let runOnePeriod_ia_phase1 (nr_survCF:NR_results) (nr_unadjdRsvs:NR_results) (as:state_ia_all) (cflow':*[]f32) (survCF':*[]f32) (rsvsUK':*[]f32) (rsvsS2':*[]f32) (rsvsIFRS':*[]f32) :
    ...
    let qx(as:state_ia_all):state_ia_all=
        let qx_arr1(i1:i32):f32=
            ...
            let qx_ann=tbl_mort_sch[as.basisNum,as.p.scheme,as.p.sex[i1],age,as.state_new.calYear]*
                tbl_mort_hi[as.p.pensLevel,age]*tbl_mort_hlt[as.p.stateOfHealth[i1],age]*
                tbl_mort_ml[as.p.mainLife,age]*tbl_mort_occ[as.p.occ[i1],age]*tbl_mort_po[as.p.poCode,age]*
                tbl_mort_pub[as.p.sector[i1],age]*tbl_mort_smk[as.p.smoker[i1],age]
            in 1-(1-qx_ann)**(1.0/12.0)
    ...
    let rsvsUK(as:state_ia_all):state_ia_all= 
        ...
        if nr_unadjdRsvs.RsvUK==[] then
            ...
            nr_unadjdRsvs.RsvUK[as.t]/nr_survCF.RsvUK[as.t]*as.intermediate.survCF_LS
    ...
    let rsvsS2(as:state_ia_all):state_ia_all=
        ...
        if nr_unadjdRsvs.RsvS2==[] then
            ...
            nr_unadjdRsvs.RsvS2[as.t]/nr_survCF.RsvS2[as.t]*as.intermediate.survCF_LS
    ...
    let store_phase1(as:state_ia_all) (cflow:*[]f32) (survCF:*[]f32) (rsvsUK:*[]f32) (rsvsS2:*[]f32) (rsvsIFRS:*[]f32):
        (state_ia_all,*[]f32,*[]f32,*[]f32,*[]f32,*[]f32)=
        ((as with state__1=as.state_new with t=as.t+1),(udne cflow as.t as.state_new.cflow),(udne survCF as.t as.intermediate.survCF_LS),
        (udne rsvsUK as.t as.state_new.rsvsUK),(udne rsvsS2 as.t as.state_new.rsvsS2),(udne rsvsIFRS as.t as.state_new.rsvsIFRS))
...
let calcRebasedResults_ia (as:state_ia_all,cflow:*[]f32,survCF:*[]f32,rsvsUK:*[]f32,rsvsS2:*[]f32,rsvsIFRS:*[]f32): --we know this only ever takes the o/p of something that returns a tuple so it can take a tuple
    (state_ia_all,*[]f32,*[]f32,*[]f32,*[]f32,*[]f32)=
        ...
        let rsvsIFRS'':*[]f32=udne rsvsIFRS as.t (sumprod  cflow'[as.t:] tbl_vs[RsvIFRS,as.t:])--this would stem from a calc, note it updates a single value
...
let runOnePol_ia (pol:data_ia) (der:derived_ia):[][]f32 =
    ...
    let init_as_postPhase0=runNPeriods_phase0 pol.termPhase0 init_as_prePhase0
    ...
    let (asPostPhase1,all_cflow_EXP',_,all_rsvsUK_EXP',all_rsvsS2_EXP',all_rsvsIFRS_EXP') = 
        runNPeriodsWithRebasing_ia der.termOS nr_survCF nr_unadjdRsvs
...
let allPols=map2 runOnePol_ia fileData_ia derived_ia
in reduce (+..+) (zeros2 2 numPeriods) allPols
